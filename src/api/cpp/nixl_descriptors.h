...
class nixlBasicDesc {
    public:
        ...
        uint32_t  world_size;  // World Size
        uint32_t  local_rank;  // locak_rank
        nixlBasicDesc() {}; // No initialization to zero
        nixlBasicDesc(const uintptr_t &addr,
                      const size_t &len,
                      const uint32_t &dev_id);

        nixlBasicDesc(const uintptr_t &addr, const size_t &len, const uint32_t &dev_id, const uint32_t &world_size, const uint32_t &local_rank);
...
class nixlBlobDesc : public nixlBasicDesc {
    public:
        ...
        nixlBlobDesc(const uintptr_t &addr, const size_t &len,
                     const uint32_t &dev_id, const std::string &meta_info,const uint32_t &world_size,const uint32_t &local_rank);
